...
/**
 * @brief 元画像から正方形状に切り出す．
 *
 * 色数のカウントを行い、256色以上使用されている場合は失敗する。
 * 高さ，幅は画像の小さい方に合わせる．
 *
 *
 * @param[in,out] ima 変換するimage_t型へのポインタ
 * @return 変換に成功した場合、引数に指定されたポインタ、失敗した場合NULLが返る。
 */


image_t *sqr_extract(image_t *img){
  int x, y;
  int width;
  int height;
  int buf = 0;
  ...
  if(width > height){
    new_img = allocate_image(height,height,img->color_type);

    if((width - height)%2 != 0){
      buf =1;
    }
    start = (width - height + buf)/2;
    for (y=0; y < height; y++){
      for (x=0; x < height; x++){
        pixcel_t *p = &img->map[y][start+x];
        new_img->map[y][x] = *p;
      }
    }
